...
class JobWithInputsForMultipleNextJobsNoWait(BaseJob):
    async def run(self):
        self.outputs = self.Outputs(id=20)
        self.next_job_inputs = [
            JobProducingOutputs.Inputs(id=i) for i in range(self.inputs.jobs_to_make)
        ]
...
class JobWithInputsForMultipleNextJobsWithWait(BaseJob):
    async def run(self):
        self.outputs = self.Outputs(id=20)
        self.next_job_inputs = RunMultipleJobs.Inputs(
            next_jobs_inputs=[
                JobProducingOutputs.Inputs(id=i)
                for i in range(self.inputs.jobs_to_make)
            ]
        )
